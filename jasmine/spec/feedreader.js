    ...
    */
	...
	describe('Initial Entries', function() {
		...
		it('should have atleast one .entry element within the .feed container', function(done) {
			...
			var entryElement = feedContainer.firstElementChild.firstElementChild; 
			expect(feedContainer.hasChildNodes()).toBe(true);
			expect(entryElement.classList.contains('entry')).toBe(true);
			done(); 
		}); 
	});
	...
	describe('New Feed Selection', function() {
		...
		it('should change content', function(done) {
			expect(currentFeed).not.toBe(allFeeds[1]); 
			loadFeed(0); 
			done(); 
		}); 
	});
